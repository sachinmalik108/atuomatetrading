...
# Get the first matching value
    ...
    while(True):
      ...
      if(formatted_now >= time3pm):
        ...
        scriptcesymbol = 'FINNIFTY' + date + month + year+ livedata  + 'CE'
        scriptpesymbol = 'FINNIFTY' + date + month + year + livedata  + 'PE'
    ...
    while(True):
        ...
        if(float(ltp_value) >sellvalue):
            ...
            if(newselltrigerprice>= trigerprice):
             ...
             # stoploss at 65% loss
        if(float(ltp_value) < 0.4*float(buyvalue)):
            orderparams = {
                "variety": "NORMAL",
              "tradingsymbol": tradingsymbol,
                "symboltoken": symboltoken,
