...
# search file
multiple = 50
tradingsymbollive = "Nifty Fin Service"
symboltokenlive = "99926037"
...
while(True):
    ...
    while(True):
      ...
      if(formatted_now == time3pm or formatted_now == time310  or formatted_now == time305 or 
        ...
        scriptcesymbol = 'FINNIFTY' + "27" + "MAR" + year+ livedata  + 'CE'
        scriptpesymbol = 'NIFTY' + "27" + "MAR" + year + livedata  + 'PE'

        # scriptcesymbol = 'SENSEX' + "25MAR" + livedata  + 'CE'
        # scriptpesymbol = 'SENSEX' + "25MAR"  + livedata  + 'PE'
    ...
    while(True):
        ...
        ltp_data=smartApi.ltpData(exch, tradingsymbol, symboltoken)
        ltp_value = ltp_data['data']['ltp'] 
        sellvalue = 1.13*(float(buyvalue))
        highersell = 1.3*(float(buyvalue))
        # print(ltp_value)
        # print(sellvalue)
        if(float(ltp_value) >sellvalue):
           
            newselltrigerprice =  math.ceil(0.87*(float(ltp_value)) * 10) / 10
            ...
            if(newselltrigerprice>= trigerprice):
             ...
             # stoploss at 60% loss
        if(float(ltp_value) < 0.55*float(buyvalue)):
            orderparams = {
                "variety": "NORMAL",
              "tradingsymbol": tradingsymbol,
                "symboltoken": symboltoken,
